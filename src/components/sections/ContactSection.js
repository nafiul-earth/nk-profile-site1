            ...
            <ul className='flex flex-col gap-4'>
              {rows.map((row) => (
                <li key={row.label} className='flex items-center gap-3'>
                  ...
                  {row.href
                    ? <a href={row.href} target={row.external ? '_blank' : undefined} rel={row.external ? 'noopener noreferrer' : undefined} className='text-sm font-medium text-white/90 hover:text-sun transition-colors'>{row.label}</a>
